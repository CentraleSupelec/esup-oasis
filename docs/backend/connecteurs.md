...
| `COD_ETP` | code étape de l'inscription, affiché sous la formation et permettant de regrouper les inscriptions d'un même parcours. | pas de ligne « Étape » |
| `COD_SIS_CUR_AMG`, `LIB_CUR_AMG` | cursus adapté : code SISE et libellé. | pas de signalement de cursus adapté |

Ces valeurs sont projetées depuis l'inscription la plus récente de l'étudiant, comme le régime d'inscription et le
témoin boursier.

Le niveau d'études affiché reste celui porté par la formation (`NIVEAU`).

##### Niveau d'études et redoublement dérivés

L'inscription porte aussi un niveau d'études (`L1`, `M2`, `BUT1`…) et un témoin de redoublement. Ces deux valeurs ne
sont **pas** calculées par le connecteur fourni : les déduire suppose de connaître le paramétrage de l'établissement
(les codes type de diplôme `cod_tpd_etb` sont propres à chaque instance, et l'interprétation du compteur
d'inscriptions à l'étape dépend des conventions locales en matière d'étalement ou de réorientation). Elles restent
donc vides par défaut, et les signalements correspondants ne s'affichent pas.

Pour les alimenter, étendez [`App\Service\SiScol\ApogeeProvider`](../../backend/src/Service/SiScol/ApogeeProvider.php)
dans `personnalisation/SiScol`, et surchargez `enrichirInscription()`. La méthode reçoit la ligne Apogée brute et
renvoie un tableau à deux clés, toutes deux facultatives :

```php
class MonApogeeProvider extends ApogeeProvider
{
    public function getProviderId(): string
    {
        return 'apogee_mon_etablissement'; // à reporter dans SI_SCOL
    }

    protected function enrichirInscription(object $row): array
    {
        return [
            'niveauDerive' => /* 'L1', 'M2', 'BUT1'… ou null */,
            'redoublant' => /* true, false, ou null si indéterminé */,
        ];
    }
}
```

Les colonnes supplémentaires nécessaires (cycle du diplôme, année dans le diplôme, type de diplôme, compteur
d'inscriptions à l'étape…) sont à ajouter à votre requête d'inscriptions, elle aussi personnalisable. Les clés
renvoyées par le cœur ne peuvent pas être écrasées : l'enrichissement ne fait qu'ajouter.

Le niveau porté par la formation reste prioritaire sur `niveauDerive` : un établissement qui renseigne `NIVEAU`
garde exactement son affichage.
